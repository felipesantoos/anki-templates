...
| 39 | Explicit `text-align: left` on `.card` | AnkiDroid and AnkiMobile inherit `text-align: center` from the WebView shell; desktop Anki does not. Back content (`.back-header`, `.summary`) was rendering centered on mobile and left-aligned on desktop. Setting the value explicitly makes the two platforms match. `.subject` and `.title` already specify `text-align: center`, so the front is unaffected. |
